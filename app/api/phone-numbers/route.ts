...
export async function POST(request: Request) {
  try {
    ...
    console.error('Error in /api/phone-numbers POST:', error)
    return NextResponse.json(
...
export async function GET() {
  try {
    ...
    // Get all phone numbers for the user's organization
    ...
    console.error('Error in /api/phone-numbers GET:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
